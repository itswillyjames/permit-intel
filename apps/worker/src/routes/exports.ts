 ...
 *
 * NOTE: In-memory storage is NOT safe on Workers (isolates restart / different isolates).
 * R2 is the canonical storage for export HTML.
 */
...
export async function handleExports(req: Request, db: Db, env: Env): Promise<Response> {
  ...
  if (req.method === 'POST' && !exportId) {
    ...
    const r2Adapter = {
      async put(key: string, value: string): Promise<void> {
        await env.EXPORTS_BUCKET.put(key, value, {
  ...
  if (req.method === 'GET' && exportId && sub === 'html') {
    ...
    // Legacy inline exports are not durable in prod
